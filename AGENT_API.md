...
소유자가 설정에서 ‘API 새 일정에도 기본값 적용’을 켠 경우에만 `POST /items`의 생략된 `public`을 공개 기본값으로, `kind: "timed"`의 생략된 `end`를 `start` + 기본 소요 시간으로 채운다. 초기에는 꺼져 있으므로 기존처럼 모든 필드를 전송해야 한다. 명시한 값이 우선하며 `null`은 생략이 아니다. 종일·날짜 미정 일정의 `end`는 계속 필수다. PATCH와 기존 일정, 조회 결과에는 영향을 주지 않는다. 같은 멱등성 키의 재시도는 설정이 바뀌어도 최초 생성 결과를 돌려준다. 설정 관리(`/api/preferences`, GET/POST)는 소유자 브라우저 세션 전용이며 에이전트 키로 변경할 수 없다.
