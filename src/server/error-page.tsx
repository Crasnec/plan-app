...
const copy: Record<number, [string, string]> = {
  ...
  401: ["로그인이 필요해요", "이 화면을 보려면 로그인해 주세요."],
  403: [
    "가입하려면 초대가 필요해요",
    "아직 계정이 없는 Google 계정이에요. 사용 중인 사람에게 초대 링크를 받아 다시 시도하거나, 전달받은 공유 링크에서 공개된 일정을 확인해 주세요.",
  ],
  404: [
    "페이지를 찾을 수 없어요",
    "주소가 바뀌었거나 더 이상 사용할 수 없는 페이지예요. 공유 링크라면 링크를 보낸 사람에게 새 링크를 요청해 주세요.",
  ],
...
export function errorPage(status: number, retryHref = "/auth/google") {
  ...
  const login = [400, 401].includes(status);
  return renderPage({
    title: `${title} · 하루의 계획`,
    children: (
      <LoginPage>
