...
export function subscribe(store: Store, userId: string, input: Subscription) {
  ...
  store.db
    ...
    .run(id, JSON.stringify(s), Date.now(), userId);
  return id;
}
export function pushWorker(
  ...
  async function tick() {
    ...
    try {
      ...
      const userIds = (
        store.db
          .prepare("SELECT DISTINCT user_id FROM subscriptions")
      ...
      for (const userId of userIds) {
        ...
        for (const o of occurrences) {
          ...
          for (const sub of subscriptions) {
            ...
            const fresh = store
              .list(userId, addDays(today(), -8), addDays(today(), 9))
              .find((x) => x.id === o.id);
            ...
            store.db
              ...
              .run(
                now + Math.min(300000, 30000 * 2 ** Number(job.attempts)),
                id,
              );
            try {
              await send(
