...
소유자로 로그인한 뒤 **설정 → API 키 관리**를 누릅니다. PC와 모바일 모두 상단에서 설정을 열 수 있고, PC 사이드바에도 설정 메뉴가 있습니다. 키 이름·만료 기간(1~365일)·권한을 선택해 발급하고, 한 번만 표시되는 원문을 안전한 곳에 저장하세요. 기본은 읽기 전용이며 비공개 일정과 메모도 읽을 수 있습니다. 같은 화면에서 만료·최근 사용·폐기 상태를 확인하고 키를 즉시 폐기할 수 있습니다.
