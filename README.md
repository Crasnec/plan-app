...
출력된 VAPID 공개키와 비밀키를 `.env`에 한 번 저장합니다. 이미 구독한 기기가 있다면 임의로 키를 바꾸지 않습니다. HTTPS에서 앱을 열고 PC와 안드로이드 각각 ‘설정 → 알림 설정 → 이 기기 알림 켜기’를 누릅니다.
...
작업 후 화면 아래에 나타나는 알림창의 **실행 취소 / 다시 실행** 버튼 또는 `Ctrl/⌘+Z`, `Ctrl/⌘+Shift+Z`를 사용합니다. 작업 알림은 닫기 버튼으로 닫을 때까지 유지되고, 링크 복사 등 일반 알림에는 되돌리기 버튼이 나오지 않습니다. 알림을 닫아도 단축키와 작업 기록은 유지됩니다. `Ctrl+Y`도 다시 실행합니다. 입력창 안에서는 텍스트 자체의 기본 되돌리기가 유지됩니다.
...
설정의 **기본 설정**에서 소요 시간(5~1440분), 완료 일정 표시·숨기기, 새 일정의 공개 기본값을 지정할 수 있습니다. 초기값은 60분·표시·비공개입니다. 서버에 저장되어 다른 기기에도 반영되며, 기존 일정은 변경하지 않습니다. 완료 숨기기는 소유자의 달력·목록에만 적용하며 공유 화면과 API 조회 결과는 바꾸지 않습니다. 소요 시간은 새 일정을 시간 지정으로 전환할 때 적용합니다.

**API 새 일정에도 기본값 적용**은 초기에는 꺼져 있습니다. 켜면 API 생성 요청에서 생략한 `public`과 시간 지정 일정의 `end`를 설정으로 채웁니다. 직접 지정한 값은 우선하며, `null`은 생략으로 취급하지 않습니다. 수정 요청에는 적용하지 않습니다.

소유자로 로그인한 뒤 **설정 → API 키 관리**를 누릅니다. 설정은 PC 사이드바와 모바일 메뉴의 휴지통 바로 위에 있습니다. PC에서 사이드바를 숨겼다면 ‘메뉴 열기’를 먼저 누르세요. 키 이름·만료 기간(1~365일)·권한을 선택해 발급하고, 한 번만 표시되는 원문을 안전한 곳에 저장하세요. 기본은 읽기 전용이며 비공개 일정과 메모도 읽을 수 있습니다. 같은 화면에서 만료·최근 사용·폐기 상태를 확인하고 키를 즉시 폐기할 수 있습니다.

전용 Bearer 키를 사용하는 `/api/agent/v1` API와 ChatGPT용 `/mcp`를 제공합니다. 읽기·쓰기·삭제 권한, 만료·폐기, 재시도 중복 방지와 요청 감사 기록을 공유합니다. 키 관리 UI와 MCP 연결 안내는 설정에 있습니다. API 호출은 [AGENT_API.md](AGENT_API.md), ChatGPT 연결 방법은 [MCP.md](MCP.md)를 확인하세요.
