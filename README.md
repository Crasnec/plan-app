...
2026-09-10 기준 타입 검사와 테스트 40개, 운영 도메인의 HTTPS 응답·OAuth 시작 리디렉션·미인증 API 접근 차단을 확인했습니다. Google 계정의 실제 로그인 완료와 PC/안드로이드에서 앱을 닫은 상태의 알림 전달은 사용자 기기에서 추가 확인해야 합니다. 로컬 테스트 통과가 모든 운영 환경의 동작을 보증하지는 않습니다.
