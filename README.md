...
**설정 → 세션 관리**에서 활성 브라우저 로그인을 확인하고 개별 로그아웃 또는 ‘다른 기기 모두 로그아웃’을 실행할 수 있습니다. 현재 로그인 세션은 **이 기기**로 표시하며 같은 물리 기기라도 브라우저·프로필이 다르면 별도 세션입니다. 로그인·최근 사용·만료 시각은 한국 시간으로 표시합니다. 기기·브라우저명은 User-Agent로 추정하며 원문이나 IP는 저장하지 않습니다. 최근 사용 기록은 약 1분 단위이고, 기존 로그인에서 수집하지 않은 정보는 ‘기록 없음’으로 표시합니다. 종료한 세션의 실시간 연결은 닫고 이후 요청을 차단합니다. API 키는 ‘API 키 관리’, MCP 연결은 ‘MCP 연결’ 메뉴에서 별도로 폐기합니다.

설정창은 왼쪽 메뉴와 오른쪽 설정 영역으로 구성됩니다. 기본 설정의 ‘취소’는 마지막 저장 이후 변경을 버리고 닫기, ‘적용’은 저장 후 유지, ‘확인’은 저장 후 닫기입니다. 메뉴를 바꿔도 입력 중인 기본값은 유지됩니다. 링크·키 발급, 연결 폐기, 로그아웃과 브라우저 알림 권한은 별도 동작 버튼으로 즉시 실행되며 하단 취소로 되돌리지 않습니다. MCP 연결 자체는 유효기한 없이 유지되고, 접근 토큰만 짧게 만료·갱신됩니다.
