...
승인 코드는 1분·1회용이며 S256 PKCE와 리디렉션·클라이언트·리소스에 묶입니다. 접근 토큰은 최대 1시간, 연결과 갱신 토큰은 최초 승인 후 30일입니다. 갱신 토큰은 매번 회전하며 재사용 감지 시 연결 전체를 폐기합니다. 접근·갱신 토큰 원문은 DB에 저장하지 않고 해시만 저장합니다. 등록 클라이언트는 만료시키지 않으며 최대 100개, 활성 연결과 API 키 합계는 20개입니다.

**설정 → API 키 관리 → `MCP · ChatGPT` 폐기**로 연결을 즉시 해제할 수 있습니다. Google 소유자 정보 변경·만료·폐기도 매 요청 검증합니다. 연결을 폐기해도 일정은 지우지 않습니다. 새 테이블은 `mcp_oauth`이고 기존 SQLite 백업에 포함됩니다. DB 백업은 토큰 관련 해시와 비공개 일정이 들어 있으므로 외부에 공개하지 마세요.
