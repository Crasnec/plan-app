...
test("Converting all recurring occurrences between timed and all-day preserves earlier dates", () => {
  ...
  try {
    const item = s.create(uid, {
      ...defaultFields("2026-09-01"),
      title: "유형 변환",
      rule: { ...base, frequency: "daily" },
    });
    let selected = s.list(uid, "2026-09-03", "2026-09-04")[0];
    s.mutate(uid, item.id, selected.key, selected.version, "all", {
      ...selected,
      kind: "timed",
    ...
    let list = s.list(uid, "2026-09-01", "2026-09-04");
    ...
    list = s.list(uid, "2026-09-01", "2026-09-04");
    assert.deepEqual(
      list.map((e) => e.start),
...
test("editing all from a completed occurrence never marks future occurrences complete", () => {
  ...
  try {
    ...
    let e = s.list(uid, "2026-09-01", "2026-09-02")[0];
    s.mutate(uid, item.id, e.key, 1, "one", { ...e, done: true });
    e = s.list(uid, "2026-09-01", "2026-09-02")[0];
    s.mutate(uid, item.id, e.key, 2, "all", { ...e, title: "전체 제목 수정" });
    const list = s.list(uid, "2026-09-01", "2026-09-04");
    assert.deepEqual(
      list.map((x) => x.done),
      [true, false, false],
    );
...
test("multi-day events overlap ranges and all-day end is exclusive", () => {
  ...
  try {
    ...
    assert.equal(s.list(uid, "2026-09-03", "2026-09-04").length, 1);
    assert.equal(s.list(uid, "2026-09-04", "2026-09-05").length, 0);
  } finally {
    s.db.close();
  }
});
test("moved recurrence from outside window appears once; original key remains stable", () => {
  ...
  try {
    ...
    const first = s.list(uid, "2026-09-01", "2026-09-02")[0];
    s.mutate(uid, item.id, first.key, 1, "one", {
      ...first,
      start: "2026-10-02",
      end: "2026-10-03",
    });
    const moved = s
      .list(uid, "2026-10-02", "2026-10-03")
...
test("completion and public exceptions are per occurrence; stale updates conflict", () => {
  ...
  try {
    ...
    const e = s.list(uid, "2026-09-01", "2026-09-03")[0];
    s.mutate(uid, i.id, e.key, 1, "one", { ...e, done: true, public: true });
    const list = s.list(uid, "2026-09-01", "2026-09-03");
    assert.equal(list[0].done, true);
    assert.equal(list[1].done, false);
    assert.equal(s.list(uid, "2026-09-01", "2026-09-03", true).length, 1);
    assert.throws(() => s.mutate(uid, i.id, e.key, 1, "one", e), /다른 기기/);
  } finally {
    s.db.close();
  }
});
test("future edits split series; earlier completed history and later exceptions survive", () => {
  ...
  try {
    ...
    let list = s.list(uid, "2026-09-01", "2026-09-06");
    s.mutate(uid, i.id, list[0].key, 1, "one", { ...list[0], done: true });
    list = s.list(uid, "2026-09-01", "2026-09-06");
    s.mutate(uid, i.id, list[4].key, 2, "one", { ...list[4], public: true });
    list = s.list(uid, "2026-09-01", "2026-09-06");
    s.mutate(uid, i.id, list[2].key, 3, "future", { ...list[2], title: "새 제목" });
    list = s.list(uid, "2026-09-01", "2026-09-06");
...
test("all-series drag from later occurrence shifts original anchor, keeps duration", () => {
  ...
  try {
    ...
    const e = s.list(uid, "2026-09-03", "2026-09-04")[0];
    s.mutate(uid, i.id, e.key, 1, "all", {
      ...e,
      start: "2026-09-04",
      end: "2026-09-06",
    });
    assert.equal(s.item(i.id, uid).start, "2026-09-02");
    assert.equal(s.item(i.id, uid).end, "2026-09-04");
  } finally {
    s.db.close();
  }
});
test("structural rule changes preserve completed nonmatching history as a single item", () => {
  ...
  try {
    ...
    const e = s.list(uid, "2026-09-02", "2026-09-03")[0];
    s.mutate(uid, i.id, e.key, 1, "one", { ...e, done: true });
    const first = s.list(uid, "2026-09-01", "2026-09-02")[0];
    s.mutate(uid, i.id, first.key, 2, "all", {
      ...first,
      rule: { ...base, frequency: "weekly", weekdays: [1] },
    });
    const history = s.list(uid, "2026-09-02", "2026-09-03");
...
test("one-occurrence deletion restores without reverting unrelated changes", () => {
  ...
  try {
    ...
    const e = s.list(uid, "2026-09-01", "2026-09-02")[0];
    s.mutate(uid, i.id, e.key, 1, "one", null, true);
    const other = s.list(uid, "2026-09-02", "2026-09-03")[0];
    s.mutate(uid, i.id, other.key, 2, "one", { ...other, done: true });
    s.restore(uid, s.trash(uid)[0].id as string);
    assert.equal(s.list(uid, "2026-09-01", "2026-09-03").length, 2);
    assert(s.list(uid, "2026-09-02", "2026-09-03")[0].done);
  } finally {
    s.db.close();
  }
});
test("future and all deletion restore exact occurrence set", () => {
  for (const scope of ["future", "all"] as const) {
    ...
    try {
      ...
      const e = s.list(uid, "2026-09-03", "2026-09-04")[0];
      s.mutate(uid, i.id, e.key, 1, scope, null, true);
      assert.equal(
        s.list(uid, "2026-09-01", "2026-09-06").length,
        scope === "future" ? 2 : 0,
      );
      s.restore(uid, s.trash(uid)[0].id as string);
      assert.equal(s.list(uid, "2026-09-01", "2026-09-06").length, 5);
    } finally {
      s.db.close();
    }
  }
...
test("time layout packs overlapping events side-by-side and separates adjacent groups", () => {
  ...
  try {
    ...
    const layout = timeLayout(s.list(uid, "2026-09-10", "2026-09-11"), "2026-09-10");
    assert.deepEqual(
